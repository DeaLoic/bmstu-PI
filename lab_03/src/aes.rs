...
impl Encryptor {
    pub fn new(key: &str) -> Self {
        ...
        let key_bytes: &[u8; 16] = &clone_into_array(key_bytes);
        if key_bytes.len() != 16 {
            panic!("Key needs to be 16 bytes long");
        } 

        return Self {
            expanded_key: key_expansion_AES128(key_bytes),
        }
    }
}
...
fn key_expansion_AES128(key_bytes: &[u8;16]) -> [[u8;4];44] {
    let mut original_key = [[0u8;4];4];
    let mut expanded_key = [[0u8;4];44];
    let N = 4;

    // Записываем ключ в таблицу 4х4 построчно
    for i in 0..16 {
    ...
    for i in 4..44 {
        ...
        if  i % N == 0 {
            ...
            rcon[2] = RC[i/N];
            rcon[3] = RC[i/N];
            temp = xor_words(&sub_word(&rot_word(&expanded_key[i-1])), &rcon);
        } else {
...
fn substitute(byte: u8, encryption: bool) -> u8 {
    ...
    col = (byte % 16).into();
    if encryption == true {
        return AES_SBOX[row][col];
    } else {
        return INVERSE_AES_SBOX[row][col];
    }
}
...
// Циклический сдвиг строк согласно индексу
...
pub fn encrypt_AES128(aes128: &Encryptor, bytes: &[u8]) -> Vec<u8> {
    if bytes.len() % 16 != 0 {
...
pub fn encrypt_block_AES128(aes128: &Encryptor, bytes: &[u8;16]) -> [u8;16] {
    let mut result = [0u8;16];

    let mut state = [[0u8;4];4];

    // state[r, x] = input[r + 4*c]
    for i in 0..16 {
...
pub fn decrypt_AES128(aes128: &Encryptor, bytes: &[u8]) -> Vec<u8> {
    if bytes.len()%16!=0 {
        panic!("Input is not multiple of 16 bytes!");
    }
...
pub fn decrypt_block_AES128(aes128: &Encryptor, bytes: &[u8;16]) -> [u8;16] {
    let mut result = [0u8;16];
